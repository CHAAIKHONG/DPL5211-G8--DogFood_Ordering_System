...
def save_data(filename, data, delimiter="|"):
    with open(filename, "w", newline='') as file:
        for row in data:
            file.write(delimiter.join(row[:7]) + "\n")  # Force only 7 fields
# manage category
def manage_category():
    file = "category.txt"
...
def display_product_list(product_data=None, status_filter=None):
    ...
    def format_row(row_data):
        return (
            ...
            f"{get_category_name_by_id(row_data[0]):<{col_widths[1]}} | "
...
def display_categories():
    ...
    # Filter out rows with missing columns
    valid_data = [row for row in cdata if len(row) >= len(headers)]
    if not valid_data:
        print("No valid category entries.")
        return

    col_widths = [
        max(len(headers[i]), max(len(row[i]) for row in valid_data)) for i in range(len(headers))
    ]
    ...
    for row in valid_data:
        ...
        for i in range(len(headers)):
            print(f" {row[i]:<{col_widths[i]}} |", end="")
        print()
    print("=" * (sum(col_widths) + len(col_widths) * 3 + 1))
...
def get_category_name_by_id(category_id):
    ...
    cdata = load_data(cfile)
    for row in cdata:
        if len(row) >= 2 and row[0] == str(category_id):
            return row[1]
    return "Category"
